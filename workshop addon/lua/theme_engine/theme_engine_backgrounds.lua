DarkThemeEngine = DarkThemeEngine or {}
DarkThemeEngine.Settings = DarkThemeEngine.Settings or {}

DarkThemeEngine.AllBackgrounds = DarkThemeEngine.AllBackgrounds or {}
DarkThemeEngine.AllBackgroundsLookup = DarkThemeEngine.AllBackgroundsLookup or {}
DarkThemeEngine._OriginalBackgroundFunctions = DarkThemeEngine._OriginalBackgroundFunctions or {
    ClearBackgroundImages = _G.ClearBackgroundImages,
    AddBackgroundImage = _G.AddBackgroundImage,
    ChangeBackground = _G.ChangeBackground,
    DrawBackground = _G.DrawBackground,
}

local DATA_DIR = "theme_engine_data"
local SETTINGS_FILE = DATA_DIR .. "/settings.json"
local BG_CACHE_FILE = DATA_DIR .. "/backgrounds_cache.json"
local MAX_BG_CACHE_BYTES = 512 * 1024
local MAX_BG_ITEMS = 2048

local StaticImages = {}
local Active = nil
local Outgoing = nil
local NextSwapTime = 0
local NextPreviewSync = 0
local LowercaseDisabled = nil
local CachedBgJSON = nil
local BackgroundCacheLoaded = false
local NormalizePath
local IsSafeBackgroundPath
local SendPreviewState

function DarkThemeEngine.SaveSettings()
    pcall(function()
        if not file.IsDir(DATA_DIR, "DATA") then file.CreateDir(DATA_DIR) end
        file.Write(SETTINGS_FILE, util.TableToJSON(DarkThemeEngine.Settings, true))
    end)
end

local function EnsureSettingsExist()
    local OLD_FILE = "dark_theme_engine_settings.txt"
    if not file.Exists(SETTINGS_FILE, "DATA") and file.Exists(OLD_FILE, "DATA") then
        local oldContent = file.Read(OLD_FILE, "DATA")
        if oldContent then
            if not file.IsDir(DATA_DIR, "DATA") then file.CreateDir(DATA_DIR) end
            file.Write(SETTINGS_FILE, oldContent)
        end
    end

    local defaults = {
        ThemeOptions = {
            Mode            = "light",
            CustomBg        = "#2b2b2b",
            CustomText      = "#e0e0e0",
            CustomAccent    = "#0078d7",
            CustomBorder    = "#555555",
            BG_Static       = false,
            BG_NoZoom       = false,
            BG_NoFade       = false,
            BG_SwapInterval = 20,
            BG_Overlay      = 0,
            EnableMusic     = false,
            Music_PlaylistMode = false,
            Music_Shuffle   = false,
            Music_Volume    = 0.6,
            MenuFont = "",
            MenuFontSize = 0,
            MenuSoundPack = "default",
            MenuSoundVolume = 0.45,
        },
        DisabledBackgrounds = {},
        DisabledMusic       = {},
        DisabledAlbums = {},
        LastSeenChangelog = "",
    }

    if file.Exists(SETTINGS_FILE, "DATA") then
        local settingsSize = file.Size(SETTINGS_FILE, "DATA")
        if settingsSize and settingsSize > 256 * 1024 then
            DarkThemeEngine.Settings = defaults
            DarkThemeEngine.SaveSettings()
            return
        end
        local content = file.Read(SETTINGS_FILE, "DATA")
        if content and content ~= "" then
            local loaded = util.JSONToTable(content)
            if loaded then
                loaded.ThemeOptions         = loaded.ThemeOptions         or {}
                loaded.DisabledBackgrounds  = loaded.DisabledBackgrounds  or {}
                loaded.DisabledMusic        = loaded.DisabledMusic        or {}
                local numericKeys = { BG_SwapInterval = true, BG_Overlay = true, Music_Volume = true, MenuSoundVolume = true }
                for k, v in pairs(defaults.ThemeOptions) do
                    if loaded.ThemeOptions[k] == nil then
                        loaded.ThemeOptions[k] = v
                    elseif numericKeys[k] and type(loaded.ThemeOptions[k]) ~= "number" then
                        loaded.ThemeOptions[k] = v
                    end
                end
                if loaded.ThemeOptions.MenuFont == nil then loaded.ThemeOptions.MenuFont = "" end
                loaded.DisabledAlbums = loaded.DisabledAlbums or {}
                DarkThemeEngine.Settings = loaded
                return
            end
        end
    end

    DarkThemeEngine.Settings = defaults
    DarkThemeEngine.SaveSettings()
end
EnsureSettingsExist()

local function LoadBackgroundsCache()
    if file.Exists(BG_CACHE_FILE, "DATA") then
        local sz = file.Size(BG_CACHE_FILE, "DATA")
        if sz and sz > MAX_BG_CACHE_BYTES then return false end
        local content = file.Read(BG_CACHE_FILE, "DATA")
        if content and content ~= "" then
            local cached = util.JSONToTable(content)
            if cached and #cached > 0 and #cached <= MAX_BG_ITEMS then
                DarkThemeEngine.AllBackgrounds = {}
                for _, bg in ipairs(cached) do
                    if bg and IsSafeBackgroundPath(bg.path) then
                        table.insert(DarkThemeEngine.AllBackgrounds, bg)
                        DarkThemeEngine.AllBackgroundsLookup[NormalizePath(bg.path)] = true
                    end
                end
                CachedBgJSON = util.TableToJSON(DarkThemeEngine.AllBackgrounds)
                return true
            end
        end
    end
    return false
end

local function SaveBackgroundsCache()
    if not file.IsDir(DATA_DIR, "DATA") then file.CreateDir(DATA_DIR) end
    CachedBgJSON = util.TableToJSON(DarkThemeEngine.AllBackgrounds)
    file.Write(BG_CACHE_FILE, CachedBgJSON)
    BackgroundCacheLoaded = true
end

NormalizePath = function(p)
    if not p then return "" end
    return string.gsub(string.lower(p), "\\", "/")
end

IsSafeBackgroundPath = function(p)
    if type(p) ~= "string" or p == "" or #p > 260 then return false end
    local norm = NormalizePath(p)
    if string.find(norm, "%.%.", 1, false) then return false end
    if not (string.match(norm, "%.jpg$") or string.match(norm, "%.jpeg$") or string.match(norm, "%.png$")) then return false end
    return true
end

local function IsKnownBackgroundPath(p)
    if not IsSafeBackgroundPath(p) then return false end
    if not DarkThemeEngine.AllBackgroundsLookup[NormalizePath(p)] then
        DarkThemeEngine.ScanBackgrounds()
    end
    return DarkThemeEngine.AllBackgroundsLookup[NormalizePath(p)] == true
end

local function IsKnownBackgroundCategory(catName)
    if type(catName) ~= "string" or catName == "" or #catName > 96 then return false end
    if #DarkThemeEngine.AllBackgrounds == 0 then DarkThemeEngine.ScanBackgrounds() end
    for _, bg in ipairs(DarkThemeEngine.AllBackgrounds or {}) do
        local bgCat = bg.category or "Gmod Background"
        if bgCat == "Gamemode: base" then bgCat = "Gmod Background" end
        if bgCat == catName then return true end
    end
    return false
end

BackgroundCacheLoaded = LoadBackgroundsCache()

local function GetThemeOptions()
    return DarkThemeEngine.Settings and DarkThemeEngine.Settings.ThemeOptions or {}
end

local function GetDisabledBackgrounds()
    return DarkThemeEngine.Settings and DarkThemeEngine.Settings.DisabledBackgrounds or {}
end

local function EnsureLowercaseDisabled()
    if not LowercaseDisabled then
        LowercaseDisabled = {}
        local disabled = GetDisabledBackgrounds()
        for k, v in pairs(disabled) do
            if type(k) == "string" and v then
                LowercaseDisabled[NormalizePath(k)] = true
            end
        end
    end
end

local function IsBackgroundDisabled(bgName)
    EnsureLowercaseDisabled()
    if not bgName then
        bgName = Active and Active.Name
    end
    if not bgName then return false end
    return LowercaseDisabled[NormalizePath(bgName)] or false
end

function DarkThemeEngine.InvalidateDisabledCache()
    LowercaseDisabled = nil
end

function DarkThemeEngine.ScanBackgrounds()
    DarkThemeEngine.AllBackgrounds = {}
    DarkThemeEngine.AllBackgroundsLookup = {}
    local handledPaths = {}

    local function AddBg(path, category, wsid, isLocal)
        if #DarkThemeEngine.AllBackgrounds >= MAX_BG_ITEMS then return end
        if not IsSafeBackgroundPath(path) then return end
        category = tostring(category or "Gmod Background")
        if #category > 96 then category = string.sub(category, 1, 96) end
        wsid = tostring(wsid or "")
        if not string.match(wsid, "^%d*$") then wsid = "" end

        local normPath = NormalizePath(path)
        if not DarkThemeEngine.AllBackgroundsLookup[normPath] then
            DarkThemeEngine.AllBackgroundsLookup[normPath] = true
            handledPaths[normPath] = true
            table.insert(DarkThemeEngine.AllBackgrounds, {
                path = path,
                category = category,
                wsid = wsid,
                isLocal = isLocal or false
            })
        end
    end

    local function IsImageFile(f)
        local lower = string.lower(f)
        return string.match(lower, "%.jpg$") or string.match(lower, "%.jpeg$") or string.match(lower, "%.png$")
    end

    local gamemodes = engine.GetGamemodes()
    local countBefore = 0

    local addons = engine.GetAddons()
    if addons then
        for _, addon in ipairs(addons) do
            if addon.mounted and addon.downloaded ~= false and addon.title then
                local isLocal = (addon.wsid == "" or addon.wsid == "0" or not addon.wsid)
                local addonFiles = file.Find("backgrounds/*", addon.title)
                if addonFiles and #addonFiles > 0 then
                    for _, f in ipairs(addonFiles) do
                        if IsImageFile(f) then
                            AddBg("backgrounds/" .. f, addon.title, tostring(addon.wsid), isLocal)
                        end
                    end
                end
                if gamemodes then
                    for _, gm in ipairs(gamemodes) do
                        if gm and gm.name then
                            local gmFiles = file.Find("gamemodes/" .. gm.name .. "/backgrounds/*", addon.title)
                            if gmFiles and #gmFiles > 0 then
                                for _, f in ipairs(gmFiles) do
                                    if IsImageFile(f) then
                                        AddBg("gamemodes/" .. gm.name .. "/backgrounds/" .. f, addon.title, tostring(addon.wsid), isLocal)
                                    end
                                end
                            end
                        end
                    end
                end
            end
        end
    end
    countBefore = #DarkThemeEngine.AllBackgrounds

    local _, legacyFolders = file.Find("addons/*", "GAME")
    if legacyFolders then
        for _, folderName in ipairs(legacyFolders) do
            local title = folderName
            local jsonPath = "addons/" .. folderName .. "/addon.json"
            local addonJsonStr = nil
            local jsonSize = file.Size(jsonPath, "GAME")
            if jsonSize and jsonSize > 0 then
                addonJsonStr = file.Read(jsonPath, "GAME")
            end
            if addonJsonStr then
                local parsed = util.JSONToTable(addonJsonStr)
                if parsed and parsed.title then title = parsed.title end
            end
            local bgFiles = file.Find("addons/" .. folderName .. "/backgrounds/*", "GAME")
            if bgFiles and #bgFiles > 0 then
                for _, f in ipairs(bgFiles) do
                    if IsImageFile(f) then
                        local normPath = string.lower("backgrounds/" .. f)
                        if not handledPaths[normPath] then
                            AddBg("backgrounds/" .. f, title, "", true)
                        end
                    end
                end
            end
            if gamemodes then
                for _, gm in ipairs(gamemodes) do
                    if gm and gm.name then
                        local gmBgFiles = file.Find("addons/" .. folderName .. "/gamemodes/" .. gm.name .. "/backgrounds/*", "GAME")
                        if gmBgFiles and #gmBgFiles > 0 then
                            for _, f in ipairs(gmBgFiles) do
                                if IsImageFile(f) then
                                    local normPath = string.lower("gamemodes/" .. gm.name .. "/backgrounds/" .. f)
                                    if not handledPaths[normPath] then
                                        AddBg("gamemodes/" .. gm.name .. "/backgrounds/" .. f, title, "", true)
                                    end
                                end
                            end
                        end
                    end
                end
            end
        end
    end
    countBefore = #DarkThemeEngine.AllBackgrounds

    local baseFiles = file.Find("backgrounds/*", "MOD")
    if baseFiles then
        for _, f in ipairs(baseFiles) do
            if IsImageFile(f) then
                local normPath = string.lower("backgrounds/" .. f)
                if not handledPaths[normPath] then
                    AddBg("backgrounds/" .. f, "Gmod Background", "", true)
                end
            end
        end
    end
    countBefore = #DarkThemeEngine.AllBackgrounds

    if gamemodes then
        for _, gm in ipairs(gamemodes) do
            if gm and gm.name then
                local gmFiles = file.Find("gamemodes/" .. gm.name .. "/backgrounds/*", "MOD")
                if gmFiles then
                    for _, f in ipairs(gmFiles) do
                        if IsImageFile(f) then
                            local normPath = string.lower("gamemodes/" .. gm.name .. "/backgrounds/" .. f)
                            if not handledPaths[normPath] then
                                AddBg("gamemodes/" .. gm.name .. "/backgrounds/" .. f, "Gamemode: " .. gm.name, "", true)
                            end
                        end
                    end
                end
            end
        end
    end

    local dataFiles = file.Find("theme_engine_backgrounds/*", "DATA")
    if dataFiles then
        for _, f in ipairs(dataFiles) do
            if IsImageFile(f) then
                local normPath = string.lower("data/theme_engine_backgrounds/" .. f)
                if not handledPaths[normPath] then
                    AddBg("data/theme_engine_backgrounds/" .. f, "Custom Backgrounds", "", true)
                end
            end
        end
    end

    local catCounts = {}
    for _, bg in ipairs(DarkThemeEngine.AllBackgrounds) do
        local cat = bg.category or "Unknown"
        catCounts[cat] = (catCounts[cat] or 0) + 1
    end

    SaveBackgroundsCache()
end

local function CustomClearBackgroundImages()
    StaticImages = {}
end

local function CustomAddBackgroundImage(img)
    if not DarkThemeEngine or not DarkThemeEngine.AllBackgroundsLookup then return end
    local normImg = NormalizePath(img)
    if not DarkThemeEngine.AllBackgroundsLookup[normImg] then
        DarkThemeEngine.AllBackgroundsLookup[normImg] = true
        table.insert(DarkThemeEngine.AllBackgrounds, { path = img })
    end
    table.insert(StaticImages, img)
end

local function ChangeBackground(currentgm, force_swap)
    if IsInGame() or IsInLoading() then return end

    local bgOpts = GetThemeOptions()
    local isStatic = bgOpts.BG_Static and true or false

    if isStatic and not force_swap then
        if not IsBackgroundDisabled() then return end
    end

    if #DarkThemeEngine.AllBackgrounds == 0 then
        Active = nil
        Outgoing = nil
        return
    end

    local validImages = {}
    local isStaticLoop = false

    if isStatic and force_swap and Active and Active.Name then
        table.insert(validImages, Active.Name)
        isStaticLoop = true
    else
        for _, bgData in ipairs(DarkThemeEngine.AllBackgrounds) do
            if bgData and bgData.path and not IsBackgroundDisabled(bgData.path) then
                table.insert(validImages, bgData.path)
            end
        end
    end

    if #validImages == 0 then
        if Active then
            Outgoing = Active
            if bgOpts.BG_NoFade then Outgoing = nil
            else Outgoing.AlphaVel = 255 end
        end
        Active = nil
        DarkThemeEngine.CallJS("if(window.SetCurrentDarkThemeBackground) window.SetCurrentDarkThemeBackground('None');")
        SendPreviewState(true)
        return
    end

    local singleMode = (#validImages == 1)
    local img = validImages[1]
    if not singleMode then img = table.Random(validImages) end

    local attempts = 0
    if Active and Active.Name and not (isStatic and force_swap) then
        while img == Active.Name and attempts < 10 and #validImages > 1 do
            img = table.Random(validImages)
            attempts = attempts + 1
        end
    end

    if Outgoing then Outgoing.mat = nil end
    Outgoing = Active
    if Outgoing then
        Outgoing.AlphaVel = bgOpts.BG_NoFade and 90000 or (isStaticLoop and 50 or 255)
    end

    local mat = Material(img, "nocull smooth")
    if not mat or mat:IsError() then Active = nil return end

    local isNoFade = bgOpts.BG_NoFade or singleMode
    local isNoZoom = bgOpts.BG_NoZoom or singleMode

    Active = {
        Ratio = mat:GetInt("$realwidth") / mat:GetInt("$realheight"),
        Size = 1, BaseSize = 1, Angle = 0,
        AngleVel = isNoZoom and 0 or -(5 / 30),
        SizeVel = isNoZoom and 0 or (0.3 / 30),
        Alpha = 255,
        mat = mat, Name = img
    }

    if Active.Ratio < (ScrW() / ScrH()) then
        local addition = ((ScrW() / ScrH()) - Active.Ratio)
        Active.Size = Active.Size + addition
        Active.BaseSize = Active.BaseSize + addition
    end

    NextSwapTime = SysTime() + (tonumber(GetThemeOptions().BG_SwapInterval) or 20)
    DarkThemeEngine.CallJS(string.format(
        "if(window.SetCurrentDarkThemeBackground) window.SetCurrentDarkThemeBackground('%s');",
        string.JavascriptSafe(img or "")
    ))
    SendPreviewState(true)
end

local function Think(tbl, isOutgoing)
    tbl.Angle = tbl.Angle + (tbl.AngleVel * FrameTime())
    local bgOpts = GetThemeOptions()

    if bgOpts.BG_NoZoom then

        tbl.Angle = Lerp(FrameTime() * 2, tbl.Angle, 0)
        tbl.Size = Lerp(FrameTime() * 2, tbl.Size, tbl.BaseSize or 1)

        if math.abs(tbl.Angle) < 0.01 and math.abs(tbl.Size - (tbl.BaseSize or 1)) < 0.001 then
            tbl.Angle = 0
            tbl.Size = tbl.BaseSize or 1
        end
    elseif bgOpts.BG_Static then
        tbl.AngleVel = 0
        tbl.Angle = Lerp(FrameTime() * 2, tbl.Angle, 0)
        if math.abs(tbl.Angle) < 0.01 then tbl.Angle = 0 end
        if not tbl.StartTime then tbl.StartTime = SysTime() end
        local elapsed = SysTime() - tbl.StartTime
        local cycleDuration = 60
        local sineVal = (math.sin((elapsed / cycleDuration) * math.pi * 2 - (math.pi / 2)) + 1) / 2
        tbl.Size = (tbl.BaseSize or 1) + (sineVal * 0.15)
    else
        tbl.Size = tbl.Size + ((tbl.SizeVel / tbl.Size) * FrameTime())
    end

    if isOutgoing and tbl.AlphaVel then
        tbl.Alpha = tbl.Alpha - tbl.AlphaVel * FrameTime()
    end
end

local function PreviewFrame(tbl)
    if not tbl or not tbl.Name then return nil end
    return {
        path = tbl.Name,
        ratio = tonumber(tbl.Ratio) or 1,
        size = tonumber(tbl.Size) or 1,
        baseSize = tonumber(tbl.BaseSize) or 1,
        angle = tonumber(tbl.Angle) or 0,
        alpha = math.Clamp(tonumber(tbl.Alpha) or 255, 0, 255),
    }
end

SendPreviewState = function(force)
    if not DarkThemeEngine._ThemePageOpen or not DarkThemeEngine._ThemeBackgroundTabOpen then return end
    local now = SysTime()
    if not force and now < NextPreviewSync then return end
    NextPreviewSync = now + 0.05

    local opts = GetThemeOptions()
    local payload = {
        active = PreviewFrame(Active),
        outgoing = PreviewFrame(Outgoing),
        overlay = tonumber(opts.BG_Overlay) or 0,
        noFade = opts.BG_NoFade and true or false,
        noZoom = opts.BG_NoZoom and true or false,
        isStatic = opts.BG_Static and true or false,
    }
    DarkThemeEngine.CallJS("if(window.SetDarkThemeBackgroundPreviewState)window.SetDarkThemeBackgroundPreviewState(" .. util.TableToJSON(payload) .. ");")
end

local function Render(tbl)
    if not tbl.mat or tbl.Alpha <= 0 then return end
    if not surface or not surface.SetMaterial then return end
    surface.SetMaterial(tbl.mat)
    surface.SetDrawColor(255, 255, 255, tbl.Alpha)

    local scale = tbl.Size * 1.02
    local w = ScrH() * scale * tbl.Ratio
    local h = ScrH() * scale
    local x = ScrW() * 0.5
    local y = ScrH() * 0.5
    surface.DrawTexturedRectRotated(x, y, w, h, tbl.Angle)
end

local function UpdateBackgroundState()
    if IsInGame() or IsInLoading() then return end
    if not DarkThemeEngine or not DarkThemeEngine.AllBackgrounds then return end

    local validCount = 0
    for _, bgData in ipairs(DarkThemeEngine.AllBackgrounds) do
        if bgData and bgData.path and not IsBackgroundDisabled(bgData.path) then
            validCount = validCount + 1
        end
    end

    local isStatic = (GetThemeOptions().BG_Static or validCount <= 1)
    if not isStatic and Active and SysTime() > NextSwapTime then ChangeBackground() end
    if Active and IsBackgroundDisabled() then ChangeBackground(nil, true) end
    if not Active and validCount > 0 then ChangeBackground(nil, true) end
end

local function CustomDrawBackground()
    if IsInGame() or IsInLoading() then return end
    if not DarkThemeEngine or not DarkThemeEngine.AllBackgrounds then return end

    if DarkThemeEngine._ThemePageOpen then
        UpdateBackgroundState()
        if Active then Think(Active, false) end
        if Outgoing then Think(Outgoing, true) end
        if Outgoing and Outgoing.Alpha <= 0 then Outgoing.mat = nil; Outgoing = nil end
        SendPreviewState(false)
        return
    end

    UpdateBackgroundState()

    if Active then Think(Active, false) Render(Active) end
    if Outgoing then Think(Outgoing, true) Render(Outgoing) end

    if Outgoing and Outgoing.Alpha <= 0 then Outgoing.mat = nil; Outgoing = nil end

    local overlay = tonumber(GetThemeOptions().BG_Overlay) or 0
    if overlay > 0 then
        surface.SetDrawColor(0, 0, 0, math.floor(overlay * 255))
        surface.DrawRect(0, 0, ScrW(), ScrH())
    end
end

function DarkThemeEngine_SetThemePageOpen(open)
    DarkThemeEngine._ThemePageOpen = open == true
    if DarkThemeEngine._ThemePageOpen and not Active and DarkThemeEngine.AllBackgrounds and #DarkThemeEngine.AllBackgrounds > 0 then
        ChangeBackground(nil, true)
    end
    if not DarkThemeEngine._ThemePageOpen then
        DarkThemeEngine._ThemeBackgroundTabOpen = false
        NextSwapTime = SysTime() + (tonumber(GetThemeOptions().BG_SwapInterval) or 20)
    end
end

function DarkThemeEngine_SetThemeBackgroundTabOpen(open)
    DarkThemeEngine._ThemeBackgroundTabOpen = open == true
    if DarkThemeEngine._ThemePageOpen and DarkThemeEngine._ThemeBackgroundTabOpen then
        UpdateBackgroundState()
        if Active and Active.Name then
            DarkThemeEngine.CallJS(string.format(
                "if(window.SetCurrentDarkThemeBackground) window.SetCurrentDarkThemeBackground('%s');",
                string.JavascriptSafe(Active.Name or "")
            ))
        end
        SendPreviewState(true)
    end
end

function DarkThemeEngine.WarmBackgroundCache()
    if IsInGame() or IsInLoading() then return end
    if BackgroundCacheLoaded and #DarkThemeEngine.AllBackgrounds > 0 then return end
    if DarkThemeEngine._WarmingBackgroundCache then return end
    DarkThemeEngine._WarmingBackgroundCache = true
    timer.Simple(0, function()
        if not IsInGame() and not IsInLoading() then
            DarkThemeEngine.ScanBackgrounds()
        end
        DarkThemeEngine._WarmingBackgroundCache = false
    end)
end

timer.Create("DarkTheme_BackgroundPreviewDriver", 0.25, 0, function()
    if not DarkThemeEngine or not DarkThemeEngine._ThemePageOpen then return end
    UpdateBackgroundState()
end)

function ForceDarkThemeBackgroundSwap()
    ChangeBackground(nil, true)
end

function DarkThemeEngine_SetActiveBackground(bgPath)
    if not IsKnownBackgroundPath(bgPath) then return end
    if IsBackgroundDisabled(bgPath) then return end
    if IsInGame() or IsInLoading() then return end

    local mat = Material(bgPath, "nocull smooth")
    if not mat or mat:IsError() then return end

    if Outgoing then Outgoing.mat = nil end
    Outgoing = Active

    local bgOpts = GetThemeOptions()
    if Outgoing then
        Outgoing.AlphaVel = bgOpts.BG_NoFade and 90000 or 255
    end

    local validCount = 0
    for _, bgData in ipairs(DarkThemeEngine.AllBackgrounds or {}) do
        if bgData and bgData.path and not IsBackgroundDisabled(bgData.path) then
            validCount = validCount + 1
        end
    end

    local w = math.max(1, mat:GetInt("$realwidth") or 1)
    local h = math.max(1, mat:GetInt("$realheight") or 1)
    local singleMode = validCount <= 1
    local isNoZoom = bgOpts.BG_NoZoom or singleMode

    Active = {
        Ratio = w / h,
        Size = 1, BaseSize = 1, Angle = 0,
        AngleVel = isNoZoom and 0 or -(5 / 30),
        SizeVel = isNoZoom and 0 or (0.3 / 30),
        Alpha = 255,
        mat = mat, Name = bgPath
    }

    if Active.Ratio < (ScrW() / ScrH()) then
        local addition = ((ScrW() / ScrH()) - Active.Ratio)
        Active.Size = Active.Size + addition
        Active.BaseSize = Active.BaseSize + addition
    end

    NextSwapTime = SysTime() + (tonumber(GetThemeOptions().BG_SwapInterval) or 20)
    DarkThemeEngine.CallJS(string.format(
        "if(window.SetCurrentDarkThemeBackground) window.SetCurrentDarkThemeBackground('%s');",
        string.JavascriptSafe(bgPath or "")
    ))
    SendPreviewState(true)
end

function ReceiveThemeDataFromJS(jsonStr, silent)
    if type(jsonStr) ~= "string" or #jsonStr > 256 * 1024 then return end
    local data = util.JSONToTable(jsonStr)
    if type(data) ~= "table" then return end

    DarkThemeEngine.Settings = data
    DarkThemeEngine.SaveSettings()
    DarkThemeEngine.InvalidateDisabledCache()

    if not silent then
        ForceDarkThemeBackgroundSwap()
    else

        if Active and Active.Name and IsBackgroundDisabled(Active.Name) then
            ForceDarkThemeBackgroundSwap()
        end

        if Active and type(Active) == "table" then
            local bgOpts = GetThemeOptions()
            local validCount = 0
            for _, bgData in ipairs(DarkThemeEngine.AllBackgrounds) do
                if bgData and bgData.path and not IsBackgroundDisabled(bgData.path) then
                    validCount = validCount + 1
                end
            end
            local singleMode = (validCount <= 1)
            local isNoZoom = bgOpts.BG_NoZoom or singleMode
            if isNoZoom then
                Active.AngleVel = 0
                Active.SizeVel = 0
            else
                Active.AngleVel = -(5 / 30)
                Active.SizeVel = (0.3 / 30)
            end
        end
    end
end

function DarkThemeEngine_ToggleBackground(bgPath)
    if not IsKnownBackgroundPath(bgPath) then return end
    DarkThemeEngine.Settings.DisabledBackgrounds = DarkThemeEngine.Settings.DisabledBackgrounds or {}
    if DarkThemeEngine.Settings.DisabledBackgrounds[bgPath] then
        DarkThemeEngine.Settings.DisabledBackgrounds[bgPath] = nil
    else
        DarkThemeEngine.Settings.DisabledBackgrounds[bgPath] = true
    end
    DarkThemeEngine.SaveSettings()
    DarkThemeEngine.InvalidateDisabledCache()

    if Active and Active.Name == bgPath and IsBackgroundDisabled(bgPath) then
        ForceDarkThemeBackgroundSwap()
    end
end

function DarkThemeEngine_EnableAllBackgrounds()
    DarkThemeEngine.Settings.DisabledBackgrounds = {}
    DarkThemeEngine.SaveSettings()
    DarkThemeEngine.InvalidateDisabledCache()
end

function DarkThemeEngine_DisableAllBackgrounds()
    DarkThemeEngine.Settings.DisabledBackgrounds = DarkThemeEngine.Settings.DisabledBackgrounds or {}
    for _, bg in ipairs(DarkThemeEngine.AllBackgrounds) do
        DarkThemeEngine.Settings.DisabledBackgrounds[bg.path] = true
    end
    DarkThemeEngine.SaveSettings()
    DarkThemeEngine.InvalidateDisabledCache()
end

function DarkThemeEngine_DisableCategoryBackgrounds(catName)
    if not IsKnownBackgroundCategory(catName) then return end
    DarkThemeEngine.Settings.DisabledBackgrounds = DarkThemeEngine.Settings.DisabledBackgrounds or {}
    for _, bg in ipairs(DarkThemeEngine.AllBackgrounds) do
        local bgCat = bg.category or "Gmod Background"
        if bgCat == "Gamemode: base" then bgCat = "Gmod Background" end
        if bgCat == catName then
            DarkThemeEngine.Settings.DisabledBackgrounds[bg.path] = true
        end
    end
    DarkThemeEngine.SaveSettings()
    DarkThemeEngine.InvalidateDisabledCache()
    if Active and Active.Name and IsBackgroundDisabled(Active.Name) then
        ForceDarkThemeBackgroundSwap()
    end
end

function DarkThemeEngine_EnableCategoryBackgrounds(catName)
    if not IsKnownBackgroundCategory(catName) then return end
    DarkThemeEngine.Settings.DisabledBackgrounds = DarkThemeEngine.Settings.DisabledBackgrounds or {}
    for _, bg in ipairs(DarkThemeEngine.AllBackgrounds) do
        local bgCat = bg.category or "Gmod Background"
        if bgCat == "Gamemode: base" then bgCat = "Gmod Background" end
        if bgCat == catName then
            DarkThemeEngine.Settings.DisabledBackgrounds[bg.path] = nil
        end
    end
    DarkThemeEngine.SaveSettings()
    DarkThemeEngine.InvalidateDisabledCache()
end

function DarkThemeEngine_SetBGOption(key, value)
    local allowed = {
        BG_Static = "boolean",
        BG_NoZoom = "boolean",
        BG_NoFade = "boolean",
        BG_SwapInterval = "number",
        BG_Overlay = "number",
    }
    local expected = allowed[key]
    if not expected then return end

    if expected == "boolean" then
        value = value == true
    elseif key == "BG_SwapInterval" then
        value = math.max(5, math.min(300, tonumber(value) or 20))
    elseif key == "BG_Overlay" then
        value = math.max(0, math.min(1, tonumber(value) or 0))
    end

    DarkThemeEngine.Settings.ThemeOptions = DarkThemeEngine.Settings.ThemeOptions or {}
    DarkThemeEngine.Settings.ThemeOptions[key] = value
    DarkThemeEngine.SaveSettings()

    if key == "BG_SwapInterval" then
        NextSwapTime = SysTime() + value
    elseif key == "BG_Static" or key == "BG_NoZoom" then
        local validCount = 0
        for _, bgData in ipairs(DarkThemeEngine.AllBackgrounds or {}) do
            if bgData and bgData.path and not IsBackgroundDisabled(bgData.path) then
                validCount = validCount + 1
            end
        end
        local bgOpts = GetThemeOptions()
        local singleMode = validCount <= 1
        local isNoZoom = bgOpts.BG_NoZoom or singleMode
        if Active then
            Active.AngleVel = isNoZoom and 0 or -(5 / 30)
            Active.SizeVel = isNoZoom and 0 or (0.3 / 30)
            if bgOpts.BG_Static then Active.StartTime = SysTime() end
        end
        if Outgoing then
            Outgoing.AngleVel = isNoZoom and 0 or -(5 / 30)
            Outgoing.SizeVel = isNoZoom and 0 or (0.3 / 30)
        end
        NextSwapTime = SysTime() + (tonumber(bgOpts.BG_SwapInterval) or 20)
    end
end

function DarkTheme_FetchWorkshopIcon(wsid)
    if not wsid or wsid == "" or wsid == "0" then return end
    wsid = tostring(wsid)
    if not string.match(wsid, "^%d+$") then return end

    steamworks.FileInfo(wsid, function(result)
        if result and result.previewid then
            steamworks.Download(result.previewid, false, function(name)
                if name then
                    local url = "../" .. name
                    DarkThemeEngine.CallJS(string.format(
                        "if(window.SetDarkThemeAddonImage) window.SetDarkThemeAddonImage('%s', '%s');",
                        wsid, string.JavascriptSafe(url)
                    ))
                end
            end)
        elseif result and result.previewurl then
            if not string.match(tostring(result.previewurl), "^https://") then return end
            DarkThemeEngine.CallJS(string.format(
                "if(window.SetDarkThemeAddonImage) window.SetDarkThemeAddonImage('%s', '%s');",
                wsid, string.JavascriptSafe(result.previewurl)
            ))
        end
    end)
end

function DarkThemeEngine.SendBackgroundsToJS()
    if not IsValid(pnlMainMenu) then return end

    local now = SysTime()
    if not CachedBgJSON or #DarkThemeEngine.AllBackgrounds == 0 then
        DarkThemeEngine.ScanBackgrounds()
        DarkThemeEngine._lastScanTime = now
    elseif not DarkThemeEngine._lastScanTime then
        DarkThemeEngine._lastScanTime = now
    end

    if not Active and #DarkThemeEngine.AllBackgrounds > 0 then
        ChangeBackground(nil, true)
    end

    local disabled = GetDisabledBackgrounds()
    local activeName = Active and Active.Name or "None"
    local bgOpts = GetThemeOptions()

    DarkThemeEngine.CallJS(string.format([[
        window._DarkTheme_Backgrounds = JSON.parse("%s");
        window._DarkTheme_DisabledBgs = JSON.parse("%s");
        window._DarkTheme_ActiveBg = '%s';
        window._DarkTheme_BgOptions = JSON.parse("%s");
        if (window.DarkThemeEngine_RenderBackgroundsUI) window.DarkThemeEngine_RenderBackgroundsUI();
    ]],
        string.JavascriptSafe(CachedBgJSON or "[]"),
        string.JavascriptSafe(util.TableToJSON(disabled)),
        string.JavascriptSafe(activeName),
        string.JavascriptSafe(util.TableToJSON({
            BG_Static       = bgOpts.BG_Static       or false,
            BG_NoZoom       = bgOpts.BG_NoZoom       or false,
            BG_NoFade       = bgOpts.BG_NoFade       or false,
            BG_SwapInterval = bgOpts.BG_SwapInterval or 20,
            BG_Overlay      = bgOpts.BG_Overlay      or 0,
        }))
    ))
end

function DarkThemeEngine.RestoreDefaultBackgroundSystem()
    local originals = DarkThemeEngine._OriginalBackgroundFunctions or {}
    if originals.ClearBackgroundImages then _G.ClearBackgroundImages = originals.ClearBackgroundImages end
    if originals.AddBackgroundImage then _G.AddBackgroundImage = originals.AddBackgroundImage end
    if originals.ChangeBackground then _G.ChangeBackground = originals.ChangeBackground end
    if originals.DrawBackground then _G.DrawBackground = originals.DrawBackground end

    timer.Remove("DarkTheme_HijackEngine")
    timer.Remove("DarkTheme_BackgroundWarmCache")
    timer.Remove("DarkTheme_AddonRefresh_Debounce")
    timer.Remove("DarkTheme_BackgroundPreviewDriver")
    hook.Remove("GameContentChanged", "DarkTheme_AddonRefresh")

    if IsValid(pnlMainMenu) and pnlMainMenu.UpdateBackgroundImages then
        pcall(pnlMainMenu.UpdateBackgroundImages, pnlMainMenu)
    end
end

timer.Create("DarkTheme_HijackEngine", 0.1, 0, function()
    if IsValid(pnlMainMenu) then
        _G.ClearBackgroundImages = CustomClearBackgroundImages
        _G.AddBackgroundImage    = CustomAddBackgroundImage
        _G.ChangeBackground      = ChangeBackground
        _G.DrawBackground        = CustomDrawBackground

        if #DarkThemeEngine.AllBackgrounds > 0 then
            pnlMainMenu:UpdateBackgroundImages()
        else
            DarkThemeEngine.ScanBackgrounds()
            pnlMainMenu:UpdateBackgroundImages()
        end

        if not BackgroundCacheLoaded then
            timer.Simple(3, function()
                if not IsInGame() and not IsInLoading() then
                    DarkThemeEngine.ScanBackgrounds()
                end
            end)
        end

        timer.Remove("DarkTheme_BackgroundWarmCache")
        timer.Create("DarkTheme_BackgroundWarmCache", 8, 1, function()
            if not IsInGame() and not IsInLoading() and #DarkThemeEngine.AllBackgrounds == 0 then
                DarkThemeEngine.ScanBackgrounds()
            end
        end)

        timer.Remove("DarkTheme_HijackEngine")
    end
end)

hook.Add("GameContentChanged", "DarkTheme_AddonRefresh", function()
    if IsInGame() or IsInLoading() then return end
    timer.Remove("DarkTheme_AddonRefresh_Debounce")
    timer.Create("DarkTheme_AddonRefresh_Debounce", 1.5, 1, function()
        if IsInGame() or IsInLoading() then return end
        DarkThemeEngine.ScanBackgrounds()
        if DarkThemeEngine.InvalidateMusicCache then DarkThemeEngine.InvalidateMusicCache() end
        if DarkThemeEngine.InvalidateMenuSoundCache then DarkThemeEngine.InvalidateMenuSoundCache() end
        if DarkThemeEngine.InvalidateSpawnmenuCache then DarkThemeEngine.InvalidateSpawnmenuCache() end
        if IsValid(pnlMainMenu) then
            pnlMainMenu:UpdateBackgroundImages()
            DarkThemeEngine.CallJS("window._DarkTheme_BgDirty = true; window._DarkTheme_MusicDirty = true; window._DarkTheme_MiscDirty = true; window._DT_InteractionMounted = false;")
        end
    end)
end)
