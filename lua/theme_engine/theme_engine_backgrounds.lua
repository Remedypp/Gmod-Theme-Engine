    ...
    if file.Exists(SETTINGS_FILE, "DATA") then
        ...
        if content and content ~= "" then
            ...
            if loaded then
                ...
                local numericKeys = { BG_SwapInterval = true, BG_Overlay = true, Music_Volume = true }
...
local function ChangeBackground(currentgm, force_swap)
    ...
    local isNoZoom = bgOpts.BG_NoZoom or bgOpts.BG_Static or singleMode
...
hook.Add("GameContentChanged", "DarkTheme_AddonRefresh", function()
    ...
    if IsValid(pnlMainMenu) then
        pnlMainMenu:UpdateBackgroundImages()
        DarkThemeEngine.CallJS("window._DarkTheme_BgDirty = true; window._DarkTheme_MusicDirty = true; window._DT_InteractionMounted = false;")
    end
end)